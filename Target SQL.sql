...
INFORMATION_SCHEMA.COLUMNS where table_name = "customers" and table_schema =  "target_data";
-- ### 2. Get the time range between which the orders were placed.
select min(o.order_purchase_timestamp) fist_purchase_time , max(o.order_purchase_timestamp) last_purchase_time, 
datediff(max(o.order_purchase_timestamp),min(o.order_purchase_timestamp)) as days from target_data.orders  o;
-- ### 3. Count the Cities & States of customers who ordered during the given period.
select distinct c.customer_state, c.customer_city, count(o.order_id) as no_of_orders from target_data.customers c left join target_data.orders o
...
from target_data.orders o left join target_data.payments p
on o.order_id = p.order_id
group by 1
order by 1 ;
-- ### 2. Can we see some kind of monthly seasonality in terms of the no. of orders being placed?
with msi as
(select extract(year from o.order_purchase_timestamp) as year,extract(month from o.order_purchase_timestamp) as month, monthname(o.order_purchase_timestamp) as month_name , count(o.order_id) as no_of_order, round(sum(p.payment_value),2) as payment_amount 
from target_data.orders o left join target_data.payments p
on o.order_id = p.order_id
group by 1,2,3
order by 1,2)
...
select 
 case 
 when time (o.order_purchase_timestamp) between "00:00:00" and "06:00:00" then "Dawn" 
 when time (o.order_purchase_timestamp) between "06:00:01" and "12:00:00" then "Morning"
 when time (o.order_purchase_timestamp) between "12:00:01" and "18:00:00" then "Afternoon"
 when time (o.order_purchase_timestamp) between "18:00:01" and "23:59:59" then "Night"
 else "NA"
end as time_of_day, count(o.order_purchase_timestamp) as order_count from target_data.orders o
group by 1
order by 2 desc ;
...
with mom as
(select c.customer_state, extract(month from o.order_purchase_timestamp) as month ,monthname(o.order_purchase_timestamp) as month_name,
...
with mom as
(select extract(year from o.order_purchase_timestamp) as year, round(sum(p.payment_value),2) as sum_of_payment from target_data.orders o left join target_data.payments p
on o.order_id = p.order_id
...
from target_data.customers c left join target_data.orders o
on c.customer_id = o.customer_id left join target_data.order_items oi
...
select o.order_id , datediff(o.order_delivered_customer_date, o.order_purchase_timestamp) as time_to_deliver,
datediff(o.order_estimated_delivery_date, o.order_delivered_customer_date) as diff_estimated_delivery  from target_data.orders o;
-- ### 2. Find out the top 5 states with the highest & lowest average freight value.
-- #### > Highest >
select c.customer_state, round(avg(oi.freight_value),2) as avg_freight_value from target_data.customers c join target_data.orders o
on c.customer_id = o.customer_id join target_data.order_items oi
on o.order_id = oi.order_id
group by 1
order by 2 desc
limit 5 ;
...
-- #### > Highest >
select c.customer_state, round(avg(datediff(o.order_delivered_customer_date,o.order_purchase_timestamp)),2) as avg_delivery_time 
from target_data.customers c right join target_data.orders o
on c.customer_id = o.customer_id
group by 1
order by 2 desc
limit 5 ;
...
select c.customer_state,round(avg(datediff(o.order_estimated_delivery_date,o.order_delivered_customer_date)),2) as avg_diff_estimated_delivery 
from target_data.customers c right join target_data.orders o
on c.customer_id = o.customer_id
group by 1
order by 2
...
with mom as
(select monthname(o.order_purchase_timestamp) as month, extract(month from o.order_purchase_timestamp) as int_month, p.payment_type , count(o.order_id) as no_of_order_placed  
from target_data.payments p left join target_data.orders o
...
(select customer_state,case
 ...
 else "Tocantis" end as name_of_state,tac.customer_city,count(o.order_id) as no_of_orders from target_data.customers tac join target_data.orders o
